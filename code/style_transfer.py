...
class Model(object):
    ...
    def __init__(self, args, vocab):
        ...
        dim_z = args.dim_z  # 500
        dim_h = dim_y + dim_z  # 700
        dim_emb = args.dim_emb  # 100
        n_layers = args.n_layers  # 1
        max_len = args.max_seq_length  # 20
        filter_sizes = [int(x) for x in args.filter_sizes.split(',')]  # 1,2,3,4,5
        n_filters = args.n_filters  # 128
        ...
        self.enc_inputs = tf.placeholder(tf.int32, [None, None],    #size * len
            name='enc_inputs')
        # dec_inputs은 뭘까? <go> 부터 시작해서 original sentence id를 input으로 가짐.
        ...
        self.weights = tf.placeholder(tf.float32, [None, None],
            name='weights')
        # labels? style label를 의미하는 것인가? ㅇㅇ
        ...
        init_state = tf.concat([linear(labels, dim_y, scope='encoder'),
            tf.zeros([self.batch_size, dim_z])], 1)  # [batch_size, dim_h(dim_y+dim_z)]
        ...
        z = z[:, dim_y:]  # content latent vector z: [batch_size, dim_z]

        self.h_ori = tf.concat([linear(labels, dim_y,
            scope='generator'), z], 1)  # original style vector + learned content vector z,
                                        # [batch_size, dim_h (dim_y + dim_z)]
        self.h_tsf = tf.concat([linear(1-labels, dim_y,
            scope='generator', reuse=True), z], 1)  # target style vector + learned content vector z
                                                    # [batch_size, dim_h (dim_y + dim_z)]

        cell_g = create_cell(dim_h, n_layers, self.dropout)
        g_outputs, _ = tf.nn.dynamic_rnn(cell_g, dec_inputs,
            initial_state=self.h_ori, scope='generator')  # [batch_size, max_step, dim_h]

        # attach h0 in the front
        teach_h = tf.concat([tf.expand_dims(self.h_ori, 1), g_outputs], 1)  # [batch_size, 1, dim_h]

        g_outputs = tf.nn.dropout(g_outputs, self.dropout)  # [batch_size, max_step, dim_h]
        g_outputs = tf.reshape(g_outputs, [-1, dim_h])  # [batch_size * max_step, dim_h]
        g_logits = tf.matmul(g_outputs, proj_W) + proj_b  # [batch_size * max_step, vocab_size]

        # targets: [batch_size, max_step]. placeholder이기 떄문에 batch size 를 dimension으로 가지지 않음.
        loss_rec = tf.nn.sparse_softmax_cross_entropy_with_logits(
            labels=tf.reshape(self.targets, [-1]), logits=g_logits)
        # weights: start token을 무시하도록 함
        loss_rec *= tf.reshape(self.weights, [-1])
        self.loss_rec = tf.reduce_sum(loss_rec) / tf.to_float(self.batch_size)

        #####   feed-previous decoding   #####
        # Soft sample (Hu et al. 2017)과 argmax 비교
        go = dec_inputs[:, 0, :]
